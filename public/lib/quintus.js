   ...
   @for Quintus
    ...
    Q._detect = function (obj, iterator, context, arg1, arg2) {
        ...
        if (obj.length === +obj.length) {
            for (var i = 0, l = obj.length; i < l; i++) {
                var result = iterator.call(context, obj[i], i, arg1, arg2);
                if (result) {
                    return result;
                }
            }
            ...
            for (var key in obj) {
                var result = iterator.call(context, obj[key], key, arg1, arg2);
                if (result) {
                    var col = {};
                    col.distance = result.distance;
                    col.magnitude = result.magnitude;
                    col.normalX = result.normalX;
                    col.normalY = result.normalY;
                    col.obj = result.obj;
                    col.separate = result.separate;
                    resultArr.push(col);
                }
            }
        }
        ...
        var realResult = resultArr[0];
        for (var m in resultArr) {
            var tempResult = resultArr[m];
            if (tempResult.obj && tempResult.obj.p && tempResult.obj.p.z && tempResult.obj.p.z > realResult.obj.p.z) {
